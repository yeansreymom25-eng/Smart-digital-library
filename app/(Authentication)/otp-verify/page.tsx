...
function OTPVerifyInner() {
  ...
  async function finalizePendingSignup(supabase: SupabaseClient) {
    ...
    if (!pendingSignup?.password) {
      setMessage("Your email was confirmed, but the signup session no longer has your password. Please create your account again from the signup page.");
  ...
  useEffect(() => {
    ...
    async function handleLinkConfirmation() {
      ...
      if (!code && !tokens) {
        ...
        if (type === "signup" && !pendingSignup?.password) return;
        if (email && sessionEmail && sessionEmail !== email.toLowerCase()) return;
      }

      if (type === "signup" && !pendingSignup?.password) return;

      setLinkModeLoading(true);
      setMessage(type === "login" ? "Signing you in from your email link..." : "Finishing verification from your email link...");

      if (code) {
        const { error } = await supabaseClient.auth.exchangeCodeForSession(code);
        if (error) {
          if (mounted) { setMessage(toFriendlyAuthMessage(error.message)); setLinkModeLoading(false); }
          return;
        }
      } else if (tokens) {
  ...
  async function handleVerify() {
    if (!email) { setMessage("Email missing. Please go back and request OTP again."); return; }
    ...
    const response = await fetch("/api/auth/verify-otp", {
      ...
      body: JSON.stringify({ email, token: finalOtp }),
    });

    const result = (await response.json()) as { success?: boolean; error?: string; role?: string };

    if (!response.ok || !result.success) {
      setMessage(toFriendlyAuthMessage(result.error ?? "Verification failed."));
      setLoading(false);
      return;
    }
  ...
  async function handleResend() {
    ...
    const response = await fetch("/api/auth/resend-otp", {
      ...
      body: (() => {
        const pendingSignup = readPendingSignup();
        return JSON.stringify({ email, type, password: pendingSignup?.password, fullName: pendingSignup?.fullName });
      })(),
    });
  ...
  useEffect(() => {
    ...
    router.replace(type === "login" ? AUTH_ROUTES.emailContinue : type === "recovery" ? AUTH_ROUTES.login : AUTH_ROUTES.signup);
  }, [email, router, type]);

  function setDigit(index: number, value: string) {
    const v = value.replace(/[^0-9]/g, "").slice(0, 1);
    setOtp((prev) => { const next = [...prev]; next[index] = v; return next; });
    if (v && index < 7) { document.getElementById(`otp-${index + 1}`)?.focus(); }
  }

  function onKeyDown(index: number, e: React.KeyboardEvent<HTMLInputElement>) {
    if (e.key === "Backspace") {
      if (otp[index]) { setOtp((prev) => { const next = [...prev]; next[index] = ""; return next; }); return; }
        ...
        <div className="auth-form-enter relative z-20 w-full max-w-md lg:flex lg:min-h-[580px] lg:flex-col lg:justify-start lg:pt-12">
          ...
          <div className="space-y-5">
            <p className="text-center text-sm leading-6 text-zinc-500">
              {type === "recovery" ? "We sent a reset verification email to " : type === "login" ? "We sent a sign-in code to " : "We sent a verification email to "}
              <span className="font-medium text-zinc-700">{email ?? "your email"}</span>. Enter the 8-digit code below.
            </p>
            ...
            <div className="flex flex-nowrap justify-center gap-1.5 sm:gap-2">
              {otp.map((digit, index) => (
                <input
                  ...
                  className="h-11 w-11 rounded-xl border border-zinc-200 bg-white text-center text-lg font-semibold text-zinc-900 outline-none transition focus:border-[#5f97ee] focus:ring-4 focus:ring-[#5f97ee]/15 sm:h-13 sm:w-13"
                />
              ))}
            </div>
            ...
            {message && (
              <p className={`text-center text-sm ${message.toLowerCase().includes("sent") ? "text-emerald-600" : "text-red-500"}`}>
                {message}
              </p>
            )}
            ...
            <Button type="button" onClick={handleVerify}
              ...
              {loading || linkModeLoading ? "Verifying..." : type === "recovery" ? "Continue" : type === "login" ? "Continue to dashboard" : "Verify account"}
            </Button>

            <div className="text-center text-xs text-zinc-500">
              <Link href={type === "login" ? AUTH_ROUTES.emailContinue : type === "recovery" ? AUTH_ROUTES.login : AUTH_ROUTES.signup}
