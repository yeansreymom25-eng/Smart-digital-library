...
function StatusBadge({ status }: { status: AdminBook["status"] }) {
  const colorClass =
    status === "Published" ? "bg-[#2ec84d]" : status === "Hidden" ? "bg-[#94a3b8]" : "bg-[#f59e0b]";
  return (
    <span className={`inline-flex min-w-[96px] justify-center rounded-[6px] px-3.5 py-1.5 text-sm font-medium text-white ${colorClass}`}>
      {status}
    </span>
  );
}
...
export default function BooksManager({ initialBooks = [] }: { initialBooks?: AdminBook[] }) {
  ...
  async function handleDeleteConfirm() {
    ...
    try {
      const res = await fetch(`/api/admin/books/${pendingDelete.id}`, { method: "DELETE" });
      if (res.ok) {
        setBooks((prev) => prev.filter((b) => b.id !== pendingDelete.id));
      }
    } catch {
      // silently fail — book stays in list
    } finally {
      setIsDeleting(false);
  ...
  return (
    <section className="space-y-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-[2.5rem] font-bold leading-none text-[#173b73]">Books Management</h1>
          <p className="mt-2 text-base text-[#4d6691]">Manage your library collection</p>
        </div>
        <Link
          ...
          className={`rounded-[8px] px-6 py-3.5 text-base font-semibold text-white shadow-[0_10px_20px_rgba(77,152,240,0.16)] transition ${
            canManageBooks && !isLimitReached ? "bg-[#4d98f0] hover:bg-[#3789ea]" : "bg-[#94a3b8] hover:bg-[#7f91a8]"
          }`}
        >
          {canManageBooks ? (isLimitReached ? "Upgrade to Add More Books" : "+ Add New Book") : "Choose a Plan First"}
        </Link>
      </div>
              ...
              <tbody>
                {filteredBooks.map((book) => (
                  <tr key={book.id} className="text-[1.1rem] text-slate-900">
                    ...
                    <td className="border-t border-[#cfcfcf] px-4 py-6"><LibraryTypeBadge libraryType={book.libraryType} /></td>
                    <td className="border-t border-[#cfcfcf] px-4 py-6">{book.type}</td>
                    <td className="border-t border-[#cfcfcf] px-4 py-6">
                      {book.type === "Free" ? "Not needed" : book.paymentQrImageSrc ? "Ready" : "Missing"}
                    </td>
                    <td className="border-t border-[#cfcfcf] px-4 py-6"><StatusBadge status={book.status} /></td>
                    <td className="border-t border-[#cfcfcf] px-4 py-6">
                      <div className="flex items-center gap-4">
                        <Link href={`/library-owner/books/${book.id}/edit`} aria-label="Edit book"><ActionIcon type="edit" /></Link>
                        <Link href={`/library-owner/books/${book.id}`} aria-label="View book"><ActionIcon type="view" /></Link>
                        <button type="button" aria-label="Delete book" onClick={() => setPendingDelete(book)}>
      ...
      {pendingDelete ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/35 px-4 py-6 backdrop-blur-[2px]">
          <div className="w-full max-w-md rounded-[22px] border border-[#ffd4d4] bg-white p-6 shadow-[0_30px_80px_rgba(15,23,42,0.24)]">
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#c74a4a]">Delete Book</p>
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-end">
              <button type="button" onClick={() => setPendingDelete(null)} className="rounded-xl border border-[#e3b9b9] bg-white px-5 py-2.5 text-sm font-semibold text-[#7f4c4c] transition hover:bg-[#fff1f1]">
                Cancel
              </button>
              <button type="button" onClick={() => void handleDeleteConfirm()} disabled={isDeleting} className="rounded-xl bg-[#d94b4b] px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-[#c93d3d] disabled:opacity-50">
